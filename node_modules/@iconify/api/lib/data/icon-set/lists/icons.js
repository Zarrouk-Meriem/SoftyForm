...
const customisableProps = Object.keys(defaultIconProps);
/**
 * Generate icons tree
 */
export function generateIconSetIconsTree(iconSet, commonChunks) {
    const iconSetIcons = iconSet.icons;
    const iconSetAliases = iconSet.aliases || Object.create(null);
    ...
    const tags = [];
    const uncategorised = [];
    const resolvedTags = Object.create(null);
    ...
    // Parse all icons
    let detectedIconStyle;
    const iconsWithStroke = new Set();
    const iconsWithFill = new Set();
    const checkIconStyle = appConfig.allowFilterIconsByStyle && appConfig.enableSearchEngine && appConfig.enableIconLists;
    ...
    if (appConfig.enableIconLists && appConfig.enableSearchEngine && iconSet.info && !iconSet.info.hidden) {
        const keywords = (result.keywords = Object.create(null));
