...
export function splitKeywordEntries(values, options) {
    const results = [];
    let invalid = false;
    const splitValues = [];
    values.forEach((item) => {
    ...
    function addToSet(items, set, allowPartial) {
        let partial;
        // Add keywords
    ...
    function addToResult(set, prefix) {
 ...
 */
function addPartialPrefix(prefix, set) {
    if (prefix.slice(-1) === '*') {
        // Wildcard entry
 ...
 * Split keyword
 */
export function splitKeyword(keyword, allowPartial = true) {
    ...
    const searches = entries.map((item) => {
        return {
            ...item,
