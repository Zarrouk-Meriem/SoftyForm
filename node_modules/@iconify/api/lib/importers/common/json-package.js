 ...
 */
export async function importIconSetFromJSONPackage(prefix, path, options = {}) {
    try {
        ...
        if (data.prefix !== prefix) {
            if (!options.ignoreInvalidPrefix) {
                console.error(`Error loading "${prefix}" icon set: bad prefix (enable ignoreInvalidPrefix option in importer to skip this check)`);
                return;
            }
            data.prefix = prefix;
        }
