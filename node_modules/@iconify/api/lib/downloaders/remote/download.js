...
import { isGitHubUpdateAvailable, isGitLabUpdateAvailable, isGitUpdateAvailable, isNPMUpdateAvailable, } from './check-update.js';
import { getDownloadDirectory } from './target.js';
/**
 * Download files from remote archive
 */
export async function downloadRemoteArchive(options, ifModifiedSince, key) {
    ...
    switch (options.downloadType) {
        ...
        case 'github': {
            if (ifModifiedSince?.downloadType === 'github' &&
        ...
        case 'gitlab': {
            if (ifModifiedSince?.downloadType === 'gitlab' &&
